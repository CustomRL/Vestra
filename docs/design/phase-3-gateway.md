...
- **A1. Session timeout after a non-1000/1001 close.** ~~The docs say only "timeout after a few minutes".~~ **Measured 2026-08-20** against a live bot (`scripts/resume-check.ts` in Vestra-Testing-Bot): after a client-initiated 4000 close, a session resumed at 5 s, 60 s and 90 s, and was gone at 120 s and 180 s. **The window is 90–120 seconds, not the "few minutes" the phrasing implies.** One sample per point, on one token, so treat it as a bound rather than a constant; a session dropped abruptly rather than closed with 4000 may differ. The interim policy stands — try resume, let op 9 `d:false` decide — but anything that stalls a reconnect past ~90 s should expect to identify.
...
- **A22. Is `zstd-stream` available to all applications and API versions, or gated?** **Answered 2026-08-20**: not gated. It negotiated and decoded normally on an ordinary bot token with no special access, on API v10.

### B. Node-version unknowns — need the CI matrix, not a one-off check

**The matrix now exists**: `.github/workflows/ci.yml` runs `22.15.0`, `24.x` and `25.x`, so the items below are checked on the floor on every push rather than needing a one-off. What remains is that the transport conformance suite (§7.3, X1–X9) cannot run everywhere — see the note at the end of §7.2.
